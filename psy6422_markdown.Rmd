...
2. Visualise how closely the government data covaried with the ONS data (i.e how did vulnerability to bias affect the government data?)
...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      # Creating sliders and labelling their input
      sliderInput("scalegvt","Scale Government Data by:",  min = 1.0, max = 26, value = c(1.0)),
...
## Creating server 
server <- function(input, output) { # Define both coeff values as reactive to a slider input
  ...
  output$distPlot <- renderPlot({ # Rendering plot
    compggp <- ggplot(NULL, aes(x = date, y = covid)) +
      geom_spline(data = onsdf, 
                  # Define ONS y value as dependent on coeff2
...
The aim of this interactive graph was to allow the user to scale the data to explore the various facets of the data. For example, if the viewer is interested in the numerical differences between the two data sets, they simply need to leave the scaling factor of both lines at 1 which will not change the data in any way. However, if they want to explore how the two metrics varied proportionally over the various stages of lockdown, they can scale the ONS data down or the government data up so that they are proportional. It could be argued that the viewer only needed to be able to scale to government data set to accomplish this (and that would be true), however, speaking frankly, I felt that being able to adjust both scales made the graph feel more engaging and let the viewer play around with the data more. 
